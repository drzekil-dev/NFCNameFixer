...
맥에서 만든 한글 파일·폴더 이름을 Windows 호환(NFC)으로 바꿔주는 macOS 메뉴바 앱입니다. (v1.0)

- **맥 → 윈도 (NFC 변환)**: 맥에서 만든 한글 이름을 Windows 호환(NFC)으로 변환 (드래그 + 폴더 자동 감시)
- _윈도 → 맥 (깨짐 복원): 코드에는 포함돼 있으나 **현재 버전에서는 비활성**(주석 처리). 필요 시 `NFCNameFixerApp.swift`에서 주석 해제._
